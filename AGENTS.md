...
- **个性化主题**：12套内置主题（含深色模式），基于 CSS 变量动态切换
...
```
MugenNewTab/
├── index.html                  # 主入口（单文件架构，包含完整 HTML + CSS）
├── manifest.json               # Chrome Extension Manifest V3 配置
├── config/                     # JSON 配置文件目录
│   ├── themes.json             # 12套主题配色配置
│   ├── searchEngines.json      # 16个搜索引擎配置
│   └── defaultData.json        # 默认应用、分类和系统 UI 图标配置
├── js/                         # JavaScript 文件目录
│   ├── app.js                  # 主应用逻辑（核心类实现：Utils, StorageManager, DataManager, AppNavigator, WallpaperManager）
...
配置位于 `config/themes.json`，包含12套主题：
